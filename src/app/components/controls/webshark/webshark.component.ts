...
export class WebsharkComponent implements OnInit, AfterViewInit {
  ...
  private async initData() {
    ...
    // Use the id of the first filtered packet, not always 1
    const firstFrameId = this.destDetailsTable.length > 0 ? +this.destDetailsTable[0].id : 1;
    this.initFrameData(firstFrameId);
    this.cdr.detectChanges();
  }
  setDefaultSelection() {
