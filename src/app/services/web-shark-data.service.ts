...
export class WebSharkDataService {
  ...
  private initFromUrlParams() {
    ...
    if (fileParam) {
      // File parameter found - set it immediately (supports directories like "dir/subdir/file.pcap")
      StaticData.captureFile = fileParam;
      if (filterParam) {
        StaticData.filter = filterParam;
      }
      // Notify after a small delay to ensure subscribers are ready
      setTimeout(() => {
        this.behavior.next({});
      }, 100);
    } else {
      // Fall back to hash-based file selection
